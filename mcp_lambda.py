...
def lambda_handler(event, context):
    try:
        ...
        if utterance:
            response = process_with_mcp_server(utterance)
        else:
...
def process_with_mcp_server(utterance):
    """Send utterance to Strands OData Agent MCP server"""
    try:
        http = urllib3.PoolManager()
        
        # Call the Strands OData Agent
