...
def process_odata_utterance(utterance):
    ...
    if 'sales order' in utterance_lower:
        if 'blocked' in utterance_lower or 'block' in utterance_lower:
            if 'credit' in utterance_lower:
                return get_credit_blocked_orders()
            elif 'delivery' in utterance_lower:
                return get_delivery_blocked_orders()
            else:
                return get_blocked_orders()
        elif 'high value' in utterance_lower or 'large' in utterance_lower:
...
def get_sales_orders():
    ...
    try:
        ...
        if response.status == 200:
            ...
            if data['success']:
                orders = data['salesOrders'][:5]  # Limit to 5 orders
...
def get_high_value_orders():
    ...
    try:
        ...
        # Call the OData query endpoint for high-value orders
        query_data = {
        ...
        return "Unable to retrieve high-value orders at this time."
