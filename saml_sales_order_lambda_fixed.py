...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Step 1: Generate SAML assertion with proper XML digital signature
...
def create_signed_saml_assertion(user_email):
    """Create a SAML assertion with proper XML digital signature"""
...
-----END PRIVATE KEY-----"""
    ...
    # Create SAML assertion content
    assertion_content = f"""<Assertion ID="{assertion_id}" IssueInstant="{issue_instant}" Version="2.0" xmlns="urn:oasis:names:tc:SAML:2.0:assertion">
    ...
    # Create proper XML digital signature using cryptography
    try:
        ...
        # Calculate digest of assertion content
        digest = hashlib.sha256(assertion_content.encode('utf-8')).digest()
        digest_b64 = base64.b64encode(digest).decode('utf-8')
        
        # Sign the assertion content with RSA
        signature = private_key.sign(
            assertion_content.encode('utf-8'),
            padding.PKCS1v15(),
            hashes.SHA256()
        )
        signature_b64 = base64.b64encode(signature).decode('utf-8')
        
        # Create signed SAML assertion with proper XML digital signature structure
        signed_assertion = f"""<Assertion ID="{assertion_id}" IssueInstant="{issue_instant}" Version="2.0" xmlns="urn:oasis:names:tc:SAML:2.0:assertion" xmlns:ds="http://www.w3.org/2000/09/xmldsig#">
    <Issuer>cognito-identity-provider</Issuer>
    <ds:Signature>
        <ds:SignedInfo>
        ...
        <ds:KeyInfo>
            <ds:X509Data>
                <ds:X509Certificate></ds:X509Certificate>
            </ds:X509Data>
        </ds:KeyInfo>
    </ds:Signature>
    <Subject>
    ...
    <AuthnStatement AuthnInstant="{authn_instant}">
        ...
        logger.info("Successfully created proper XML digital signature with cryptography")
        
    except Exception as e:
        logger.error(f"Cryptography signing failed: {e}, using unsigned assertion")
        signed_assertion = assertion_content
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if response.status == 201:
                ...
                return {
                    'Result': result,
                    'Salesord': salesord,
                    'Method': 'SAML Bearer Grant with XML Digital Signature'
                }
            else:
